...
We also show, that with changing the semantics of the algorithm, especially focusing on memory consumption, we are able to improve the original
scoring (based of a composite of compute time and memory) to around one order of magnitude better on the most expensive component (read/write).
}
...
Original algorithm scales with a factor of 2.3, presized buffers by a factor of 2.14, StreamByteArray scales by 2.36
...
Scaling across file sizes: the original algorithm uses a lot of memory, and as the VM has to call more often the Garbage Collection, the cost of GC affects visibly the
scalability of the original algorithms.
As GC happens in the Compacting algorithms, the cost of it doesn't affect runtime and reducing allocations in both of the techniques of "Presized Buf" and "StreamByteArray" do show a much
better runtime profile:

So for an increase of input of 8 times, the original algorithm would slow down 31.46 times, the "Presized buffers" will slow down 17.92 times, "StreamByteArray" will slow down 18.83 times,
but Compact algorithm will have almost a perfect 8.33 times slow down.
...
Compared with elPrep paper which measure process memory size: we measure live data size, not per process memory usage.

Because of VisualVM, we can get a very precise memory consumption and many variables are excluded like: the G1GC (the default GC algorithm in Oracle JDK/OpenJDK) uses background threads to free memory which
interact with threads of execution, having a quite random values on runtime.

It is also important to understand that the more of live memory is used and the more complex the graph of objects is done and the less free memory exists on a particular machine, the GC impact will be higher.
A hypothetical GC algorithm could be triggered more seldom if it has double the free memory, because the full heap space necessary to trigger GC is occupied more seldom.

As for defaults: G1GC would automatically trigger according to documentation at 45 percent memory usage mark, and when the heap is arriving at this level of occupancy, will span background threads to do the marking phase,
so having a larger machine with double of RAM would trigger GC at least 2 times as seldom, even the GC time would take similar times given the similar number of objects (According to docs: -XX:InitiatingHeapOccupancyPercent=45)
...
Compact algorithms can process on a 48 GB machine larger files like the 8 GB BAM file and uses: 24 283 MB of live data and the 12 GB BAM input is using 35 265 MB of memory.

Even the live memory set is showing the peak memory usage (by suspending the program and measuring when the whole file is read), on practical terms all algorithms just before finishing (let's say at 99.9 percent mark of finishing reading), would
use a bit more live memory but not too much and it is quite hard (if not impossible) to measure it.
For example in case of ElPrep, as it creates millions of tasks even for smallest 144 MB BAM file, Java Streams library will create a queue of tasks that will not be freed very likely until the very end,
adding probably few tenths of maybe 100 MB of memory.
Similarly the compact algorithms would keep in memory last set of chunks (a batch) that would be similarly even on a large machine under 100 MB and they will be unused right after the
related strings are converted into data.
...
It is important to see if given a larger file, if the GC becomes a large impediment and up-to the 8 GB file, the algorithm seems to scale nicely and to be limited by other components,
like the SATA controller speed and the disk drive itself (or maybe OS components).
