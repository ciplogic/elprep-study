...
There are limits on the testing that could be done with the techniques used by this paper given the hardware differences that are between the papers: instead of a 384 GB of memory and dual-CPU server,
the most powerful machine that was available was a 8 times less RAM, with half of CPU cores, a much
slower disk drive and so on.
...
to get (at least it looks reasonable that) better numbers that are obtained than here.
...
On very small files, there is a large variation between runtimes as they run in seconds.
...
Both compact algorithms (Compact and Compact-Parallel) keep a steady pace of around 180 MB/second (averaged and it includes both read/write, higher bars are better):
...
Compact algorithms show in particular that a column-based store with heterogeneous algorithms tuned for the data type
work especially well when processing SAM files and they should work reasonably well for many data sets with CSV like formats.
Also, these algorithms can be parallelized and this parallelism is comparable (if not outright beating) in performance
row-based models. What is more important though for Java in particular, is that Column-Based models do remove on every
row the associated Java Object Headers, which translate even without heterogenous compaction into less work for GC, fewer
objects to be collected, and so on, which should translate into smaller pauses.
