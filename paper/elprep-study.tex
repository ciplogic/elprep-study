...
\\ \textcolor{brown}{
	...
	Time is 300 seconds as the machine touches Swap file, but being able to process 12 GB BAM file on a 48 GB machine is a feature in itself.
...
As we didn't have access to original large machine, we did most of testing with smallest file (the 144 MB BAM file). As results in the original paper are from the 8 GB file our result is only an extrapolation of the original paper.
...
So, in the original implementation, they claim to do the processing of the file in: 6 min:54sec to 7 min:31sec and memory usage is in 330-340 GB.
